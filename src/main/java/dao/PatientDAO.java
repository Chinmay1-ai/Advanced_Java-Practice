...
public class PatientDAO {
	...
	public static List<Patient> getAllPatients() {
		...
		try {
			...
			String sql = "SELECT p.patient_id, u.full_name, u.email, "
			...
			while (rs.next()) {
				...
				Patient p = new Patient(rs.getInt("patient_id"), rs.getString("full_name"), rs.getString("email"),
						rs.getString("gender"), rs.getString("dob"), rs.getString("blood_group"), rs.getString("phone"),
	...
	public static Patient getPatientById(int patientId) {
		...
		try {
			...
			String query = "SELECT p.patient_id, u.full_name, u.email, "
					+ "p.gender, p.dob, p.blood_group, p.phone, p.address " + "FROM patients p "
	...
	public static boolean updatePatient(Patient p) {
		...
		try {
			...
			String sql = "update patients set gender=?, dob=?, blood_group=?, phone=?, address=? where patient_id=?";
	...
	public static void deletePatient(int patientId) {
		...
		try {
			...
			// Step 1: get user_id
			PreparedStatement ps1 = con.prepareStatement("SELECT user_id FROM patients WHERE patient_id=?");
	...
	public static int getPatientIdByUserId(int userId) {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT patient_id FROM patients WHERE user_id=?");
